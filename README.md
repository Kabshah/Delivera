<div align="center">
  <img src=".github/assets/banner.svg" alt="Delivra — schedule WhatsApp messages that send automatically, even while your phone sleeps" width="100%"/>
</div>
...
<div align="center">
  ...
  <a href="https://github.com/Kabshah/Delivera/releases/latest/download/Delivra-arm64-v8a.apk">
    <img src=".github/assets/btn-arm64.svg" alt="Download Delivra for arm64-v8a devices" height="72"/>
  </a>
  <br>
  <sub>Samsung · Xiaomi · OnePlus · Pixel · Redmi · Realme · Vivo · Oppo — every phone since ~2017</sub>
  <br>
  <a href="https://github.com/Kabshah/Delivera/releases/latest/download/Delivra-armeabi-v7a.apk">
    <img src=".github/assets/btn-v7a.svg" alt="Download Delivra for armeabi-v7a devices" height="60"/>
  </a>
  <br>
  <sub>Older / entry-level 32-bit phones only. Both buttons always fetch the <b>newest release</b> —<br>the original v1.0 lives in <a href="../../releases">past releases</a>.</sub>
</div>
...
| **How** | Links to your WhatsApp as a *companion device* — the same protocol as WhatsApp Web — and sends through your own account |
| **Where your data lives** | Entirely on your phone: schedules, session keys, attachments. There is **no backend server**, no analytics, no telemetry |
| **Battery philosophy** | Connect-on-demand: the app runs only for ~2 minutes around each scheduled send. The rest of the time it doesn't exist |
...
5. Flip the **Reliable delivery** toggle on the Home screen — this is what makes scheduled sends survive a locked screen
...
- **Delivery is near-scheduled, not second-perfect.** Messages normally go out within minutes of
  the target time; aggressive OEM power management (looking at you, Samsung) can occasionally delay wakeups.
- **The Reliable delivery toggle matters.** Android cuts network access in deep sleep — without the
  exemption, screen-off sends fail. Allowing it costs no extra battery: Delivra still wakes only for its own sends.
- **Linked sessions can expire** after long periods without any WhatsApp activity (historically ~14 days).
- **Attachments must stay reachable.** Files are read at send time; deleting or moving one before its
  schedule produces a clear *"source file unavailable"* failure instead of a silent drop.
- **Crash mid-send ⇒ "Needs Review".** An ambiguous outcome is surfaced for manual checking rather
  than risking an unnoticed duplicate.
...
| `READ_EXTERNAL_STORAGE` (≤ Android 12) | Legacy attachment read path |
